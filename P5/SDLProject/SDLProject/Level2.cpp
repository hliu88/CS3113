...
void Level2::Initialize() {
    ...
    state.player->position = glm::vec3(6, -9, 0);
    state.player->startPt = glm::vec3(6,-9,0);
...
void Level2::Update(float deltaTime) {
    ...
    state.player->CheckDamage(state.enemies, LEVEL2_ENEMY_COUNT);
    if(state.player->position.y >= 0 && state.player->position.x > 1.5 && state.player->position.x < 2.5){
        state.nextScene = 3;
    }
}
